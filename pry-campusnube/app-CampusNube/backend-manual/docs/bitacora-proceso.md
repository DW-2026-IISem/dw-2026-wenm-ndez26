...
``` bash
git add . git commit -m "chore: install production dependencies for ca backend"
```
...
``` bash
git add . git commit -m "chore: install auth and sequelize-cli devDependencies"
```
...
``` bash
git add . git commit -m "chore: add scripts/free-port.js to avoid EADDRINUSE"
```
...
``` bash
git add . git commit -m "chore: wire free:port into nest start scripts"
```
